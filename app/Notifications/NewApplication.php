...
class NewApplication extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->line('You have a new Application from '. $this->application->first_name)
            ->line('for ' . $this->application->car->state . ' '. $this->application->car->make . ' ' . $this->application->car->model . ' ' . $this->application->car->year . '.')
            ->action('Click here to review the application', url('/dashboard/applications/'. $this->application->id));
    }
